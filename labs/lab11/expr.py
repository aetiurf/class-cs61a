...
class CallExpr(Expr):
    ...
    def eval(self, env):
        ...
        """
        "*** YOUR CODE HERE ***"
        operator, operands = self.operator.eval(env), [operand.eval(env) for operand in self.operands]
...
class Number(Value):
    ...
    def apply(self, arguments):
        raise TypeError("Oof! Cannot apply number {} to arguments {}".format(self.value, comma_separated(arguments)))
...
class PrimitiveFunction(Value):
    ...
    def apply(self, arguments):
        for arg in arguments:
            if type(arg) != Number:
                raise TypeError("Invalid arguments {} to {}".format(comma_separated(arguments), self))
