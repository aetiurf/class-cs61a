...
cs61a = {"Homework": 2, "Lab": 1, "Exam": 50, "Final": 80, "PJ1": 20, "PJ2": 15, "PJ3": 25, "PJ4": 30, "Extra credit": 0}
...
def make_glookup(class_assignments):
    ...
    sum_score, real_score = 0, 0

    def glookup(str, score):
        nonlocal sum_score, real_score
        sum_score += class_assignments[str]
        real_score += score
        return real_score / sum_score
...
def num_trees(n):
    ...
    *   *        *       *
    ...
    if n == 1:
        return 1
    return sum([num_trees(m) * num_trees(n - m) for m in range(1, n)])
