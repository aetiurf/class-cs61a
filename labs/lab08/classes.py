...
class ProfessorCard(Card):
    ...
    def effect(self, other_card, player, opponent):
        ...
        opp_deck = opponent.deck.cards
        tmp_deck = opponent.deck.cards[:]
        for i in range(len(opponent.deck.cards)):
            if other_card.attack == opp_deck[i].attack or other_card.defense == opp_deck[i].defense:
                tmp_deck.remove(opp_deck[i])
