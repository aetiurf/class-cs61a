...
class Frame(object):
    ...
    def make_child_frame(self, formals, vals):
        ...
        if formals is not nil or vals is not nil:
            raise SchemeError('make_child_frame Error!')
        return child
        # END PROBLEM 10
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        # BEGIN PROBLEM 5
        "*** YOUR CODE HERE ***"
        env.define(target, scheme_eval(expressions.rest.first, env, True))
        return target
        # END PROBLEM 5
...
def do_lambda_form(expressions, env):
    ...
    def to_list(x):
        if x is nil:
            return nil
        return Pair(x.first, to_list(x.rest))

    body = to_list(expressions.rest)
...
def do_and_form(expressions, env):
    ...
    if x is nil:
        return True
    while x.rest is not nil:
        if is_false_primitive(scheme_eval(x.first, env)):
            return scheme_eval(x.first, env)
        x = x.rest
    return scheme_eval(x.first, env)
    # END PROBLEM 12
...
def do_or_form(expressions, env):
    ...
    if x is nil:
        return False
    while x.rest is not nil:
        if is_true_primitive(scheme_eval(x.first, env)):
            return scheme_eval(x.first, env)
        x = x.rest
    return scheme_eval(x.first, env)
    # END PROBLEM 12
